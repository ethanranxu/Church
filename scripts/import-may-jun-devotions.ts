...
async function importDevotions() {
    console.log('========================================');
    console.log('      5-6月灵修数据增量导入工具');
    console.log('========================================\n');

    // 读取 markdown 文件
    const filePath = 'D:\\Downloads\\每日与主同行—2026年灵修（5-6月）.md';
    let markdownContent;
    try {
    ...
    // 列出所有找到的日期
    console.log('日期分布:');
    const mayDates = devotions.filter(d => d.publishDate.startsWith('2026-05')).map(d => parseInt(d.publishDate.slice(-2)));
    const junDates = devotions.filter(d => d.publishDate.startsWith('2026-06')).map(d => parseInt(d.publishDate.slice(-2)));
