...
async function importDevotions() {
    console.log('========================================');
    console.log('      7-8月灵修数据增量导入工具');
    console.log('========================================\n');

    // 读取 markdown 文件
    const filePath = path.join(__dirname, '..', '每日与主同行—2026年灵修（7-8月）.md');
    let markdownContent;
    try {
    ...
    // 列出所有找到的日期
    console.log('日期分布:');
    const julDates = devotions.filter(d => d.publishDate.startsWith('2026-07')).map(d => parseInt(d.publishDate.slice(-2)));
    const augDates = devotions.filter(d => d.publishDate.startsWith('2026-08')).map(d => parseInt(d.publishDate.slice(-2)));
    console.log(`  7月: ${julDates.join(', ')}`);
    console.log(`  8月: ${augDates.join(', ')}\n`);

    // 删除数据库中7月和8月的灵修文章
    console.log('正在删除数据库中7月和8月的灵修文章...');
    const snapshot = await db.collection('Articles').get();
    let deleteCount = 0;
    for (const doc of snapshot.docs) {
        const data = doc.data();
        const publishDate = data.publishDate;
        if (publishDate && (publishDate.startsWith('2026-07') || publishDate.startsWith('2026-08'))) {
